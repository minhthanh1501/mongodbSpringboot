package com.mongodb.MongodbSpring.config;
...
public class SercurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http, HttpSecurity httpSecurity) throws Exception {
        ...
        // requestMatchers để cấu hình endpoint users Post là được truy cập
        // các request khác phải authentication

        httpSecurity.oauth2ResourceServer(oauth2 -> oauth2.jwt(jwtConfigurer -> jwtConfigurer.decoder(jwtDecoder())));

        // httpSecurity.csrf(httpSecurityCsrfConfigurer -> httpSecurityCsrfConfigurer.disable());
        httpSecurity.csrf(AbstractHttpConfigurer::disable); // cách ngắn gọn
        // tắt csrf
